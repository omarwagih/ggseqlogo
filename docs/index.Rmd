...
- `seqs_dna`: a list of sequences for transcription factor binding sites
- `seqs_aa`: a list of sequences for kinase-substrate binding data
- `pfms_dna`: a list of position frequency matrices for transcription factors

## Plot a sequence logo 

You can draw a sequence logos using `ggplot` function, with `geom_logo`. Let's try this on sequences for one of the transcription factors:
...
ggseqlogo( pfms_dna$sample_pfm_1 )
```
...
p1 = ggseqlogo( seqs_dna$sample_dna_1, method = 'bits' )
p2 = ggseqlogo( seqs_dna$sample_dna_1, method = 'prob' )
...
Lets try generate an amino acid sequence logo:

```{r aa_example, fig.cap='An example sequence logo for amino acids'}
ggseqlogo( seqs_aa$sample_aa_1, seq_type='aa' )
```

## Custom alphabet
If you want to define a custom alphabet you can do so by setting `seq_type` to "other" and set `namespace` with your desired custom alphabet. For example, lets say you wanted a sequence logo of zeros and ones:

```{r custom_namespace, fig.width=6, fig.height=4, fig.cap='Generating sequence logos with custom dictionaries'}
seqs_other = c('10010', '10100', '01010', '01001')
ggseqlogo(seqs_other, method='p', seq_type='other', namespace=0:1) 
```
...
ggseqlogo(seqs_dna$sample_dna_1, col_scheme='base_pairing')
```
...
ggseqlogo(seqs_dna$sample_dna_1, col_scheme=cs1)
```
...
# Generate sequence logo
ggseqlogo(seqs_dna$sample_dna_1, col_scheme=cs2)
```
...
p_list = lapply(fonts, function(f){
  ggseqlogo(seqs_dna$sample_dna_1, font=f) + ggtitle(f)
})
...
ggplot() + 
  annotate('rect', xmin = 4.5, xmax = 12.5, ymin = -0.05, ymax = 1.9, alpha = .1, col='black', fill='yellow') +
  geom_logo(seqs_dna$sample_dna_4, stack_width = 0.90) + 
  annotate('segment', x = 21, xend=24, y=1, yend=1, size=2) + 
  annotate('text', x=23, y=1.2, label='Text annotation') + 
  theme_logo()
```
